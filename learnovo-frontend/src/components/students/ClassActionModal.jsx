...
const ClassActionModal = ({ isOpen, onClose, onConfirm, type, student, isLoading }) => {
    ...
    // Fetch actual class list from API
    useEffect(() => {
    ...
    // Pre-select next/prev class based on school's actual class list
    useEffect(() => {
        if (classOptions.length === 0) return;
        const index = classOptions.indexOf(student.class);
        if (index !== -1) {
            if (type === 'promote' && index < classOptions.length - 1) {
                setTargetClass(classOptions[index + 1]);
            } else if (type === 'demote' && index > 0) {
                setTargetClass(classOptions[index - 1]);
            }
        }
    }, [student, type, classOptions]);
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block transform overflow-hidden rounded-lg bg-white dark:bg-[#1C1C1E] text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                    <div className="bg-white dark:bg-[#1C1C1E] px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="sm:flex sm:items-start">
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg font-medium leading-6 text-gray-900 dark:text-white capitalize">
                                    {type} Student
                                </h3>
                                <div className="mt-2 text-sm text-gray-500 dark:text-[#8E8E93] mb-4">
                                ...
                                {showOverrideWarning && (
                                    <div className="mb-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-300 px-4 py-3 rounded-md flex items-start gap-3">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4 text-left">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="mt-1 block w-full rounded-md border border-gray-300 dark:border-[#38383A] bg-white dark:bg-[#1C1C1E] text-gray-900 dark:text-white py-2 pl-3 pr-10 text-base focus:border-primary-500 focus:outline-none focus:ring-primary-500 sm:text-sm"
                                            >
                                                <option value="">Select Class...</option>
                                                {classOptions.map(c => (
                                                    <option key={c} value={c} disabled={c === student.class}>{c}{c === student.class ? ' (current)' : ''}</option>
                                                ))}
                                            </select>
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 dark:text-[#8E8E93]">Academic Year *</label>
                                        <input
                                            type="text"
                                            required
                                            value={academicYear}
                    ...
                    <div className="bg-gray-50 dark:bg-[#2C2C2E] px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button
                            type="button"
                        ...
                        <button
                            ...
                            onClick={onClose}
                            disabled={isLoading}
                            className="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 dark:border-[#38383A] bg-white dark:bg-[#1C1C1E] px-4 py-2 text-base font-medium text-gray-700 dark:text-[#8E8E93] shadow-sm hover:bg-gray-50 dark:hover:bg-[#2C2C2E] focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                        >
                            Cancel
                        </button>
                    </div>
