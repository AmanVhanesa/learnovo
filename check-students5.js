...
mongoose.connect(uri).then(() => {
    ...
    console.log('\n--- Testing Section A ---');
    request(app)
        .get('/?class=1&section=A')
        .expect(200)
        .end((err, res) => {
            if (err) throw err;
            console.log(`Results length for A: ${res.body.length}`);
            res.body..forEach(s => console.log(s.fullName, s.section, s.sectionId));

            console.log('\n--- Testing Section D ---');
            request(app).get('/?class=1&section=D').end((err2, res2) => {
                console.log(`Results length for D: ${res2.body.length}`);
                res2.body..forEach(s => console.log(s.fullName, s.section, s.sectionId));
