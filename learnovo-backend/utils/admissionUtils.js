...
async function generateAdmissionNumber(tenantId, session = null) {
  ...
  const { mode, prefix, yearFormat, counterPadding: _counterPadding } = admissionSettings;

  const currentYear = new Date().getFullYear().toString();
  const _yearStr = yearFormat === 'YY' ? currentYear.substring(2) : currentYear;
  const _effectivePrefix = mode === 'CUSTOM' ? (prefix || 'ADM') : 'ADM';
  ...
  // Changed to return plain numeric sequence based on user request
