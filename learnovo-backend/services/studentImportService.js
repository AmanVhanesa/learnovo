...
class StudentImportService {
  ...
  /**
     ...
     */
  static async previewImport(fileOrPath, tenantId, _options = {}) {
    try {
      let rows;
      ...
      const { validRows, invalidRows: _invalidRows, errors } = ImportExportService.validateRows(rows, schema);
  ...
  static async executeImport(validData, tenantId, options = {}) {
    const {
      updateExisting: _updateExisting = false,
      skipErrors = true
    } = options;
