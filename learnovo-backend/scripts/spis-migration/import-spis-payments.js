...
async function importStudentReceipts({ tenant, session, student, receipts, execute, adminUserId }) {
  ...
  invoiceState.forEach(s => { s.balance = roundToRupee(s.totalAmount - s.paidAmount - s.discountAmount); });
  ...
  for (const receipt of receipts) {
    ...
    if (amountToSettle <= 0) { skippedReceipts++; continue; }
    ...
    if (allocatedToInvoices.length === 0) { skippedReceipts++; continue; }
  ...
  if (recurringPattern && invoiceState.length > 1) {
    const sortedInvoices = invoiceState.slice().sort((a, b) => 0); // already in periodStart order from query
    let isFirst = true;
    for (const inv of sortedInvoices) {
      if (isFirst) { isFirst = false; continue; } // skip Q1
...
(async () => {
  ...
  if (!session) { console.error(`Session ${args.session} not found`); process.exit(1); }
  console.log(`✓ Session: ${session.name}`);

  const adminUser = await User.findOne({ tenantId: tenant._id, role: 'admin' }).select('_id').lean();
  if (!adminUser) { console.error('No admin user found in SPIS'); process.exit(1); }
